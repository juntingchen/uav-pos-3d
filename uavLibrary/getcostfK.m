function [fmin, lopt, Xopt3] = getcostfK(PosUAV, PosUE, PosBS, los, U, fun)
...
% NOTE: Assume user height = 0.
    ...
    lmax = (rho * L * cos(theta) + h * Hb) / sqrt(rho^2 + h^2);
    tol = (lmax + lmin) / 2 * 1e-8;
    cnt = 0;
    MAXLOOP = 1000;
    if lmax - lmin <= tol
        l = lmin;
    ...
    while lmax - lmin > tol && cnt < MAXLOOP
        ...
        l = (lmax + lmin) / 2;
        dl = (lmax - lmin) * 1e-6;
        f_plus = fl(l + dl, rho, theta, L, h, Hb, U, fun, los);
        f_minus = fl(l - dl, rho, theta, L, h, Hb, U, fun, los);
    ...
    lopt = l;
    fmin = fl(lopt, rho, theta, L, h, Hb, U, fun, los);
