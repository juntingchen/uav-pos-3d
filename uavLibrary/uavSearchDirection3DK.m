function D = uavSearchDirection3DK(PosUAV, PosUE, PosBS, ksegment, ksearch, U, fun)
...
function f = F(rho, theta, L, h, Hb, U, fun, ksearch)
    ...
    while lmax - lmin > tol && cnt < MAXLOOP
        ...
        l = (lmax + lmin) / 2;
        dl = (lmax - lmin) * 1e-6;
        f_plus = fl(l + dl, rho, theta, L, h, Hb, U, fun, ksearch);
        f_minus = fl(l - dl, rho, theta, L, h, Hb, U, fun, ksearch);
