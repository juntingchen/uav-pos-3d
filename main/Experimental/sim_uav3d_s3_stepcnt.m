...
%   - evaluate the search complexity as suggested by the reviewer. 
%   - We focus on Scenario 3.
%
...
simCaseID = 803;
simFileNameToken = 'uavpos3d_mmwrate_stepcnt';
...
U.Hmax = 120;
U.Hdrone = 120;  % meter, UAV search height
...
parfor i = 1:Nue
    ...
    stepCounts = zeros(1, N_scheme);
%     try
        if sim_scheme(2)
            [FminStat, XhatStat, stepCntStat] = ...
...
%                 finduavpos3d_matexhst3(PosUE, PosBS, U, funs, stepSizeMeter, Maps, meterPerPixel, map_x0);
            L = norm(PosBS(1:2) - PosUE(1:2));
            stepCnt4 = ceil(pi * (L/2)^2 / (stepSizeMeter^2));
            Fmin_exhst = 0;
        else
            Fmin_exhst = 0;
        end
        
        stepCounts = [0, stepCntStat, stepCnt1, stepCnt2, stepCnt3, stepCnt4];
        stepCntMat(i, :) = stepCounts;
...
stepCounts = mean(stepCntMat, 1),

searchLen = stepCounts * stepSizeMeter,
