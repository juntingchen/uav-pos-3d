% Adapted from sim_uav3d_s3.m
%   Consider antenna pattern as suggested by the reviwer.
%   The antenna pattern ffrom Table A.2.1.1-2 in TR36.814
%
%   - We only study Scenario 3 (with no shadowing)
%   - For the simplicity of the code, we record the data to the case of 2D
%   optimization.
%
...
simCaseID = 0;
simFileNameToken = 'uavpos3d_antennapattern';

Nue = 1000; % 10000 = 5.8 hours (all schemes)
PosBS = [150, 770];
SCENARIO = 3;
...
plmmw = cell(3, 1); r0_mmw = cell(3, 1); r1_mmw = cell(3, 1); r0_mmw_ant = cell(3, 1);  
...
for k = 1:3
    ...
    % Rate with antenna backoff
    r0_mmw_ant{k} = @(d,z) mymin(log2(1 + 10.^((snr0(d) - snr_backoff + z)/10)), modulation_cutoff);
    % where the variable capture the antenna gain (0 dB means not loss)
...
if SCENARIO == 3
    ...
    % Rate with Antenna backoff
    funs_ant{1} = @(x,y,z) - 0.5 * mymin(r1_mmw{1}(x) * bw_mmw, r0_mmw_ant{1}(y,z) * bw_mmw) / 1e9;   % Tbit
    funs_ant{2} = @(x,y,z) - 0.5 * mymin(r1_mmw{2}(x) * bw_mmw, r0_mmw_ant{1}(y,z) * bw_mmw) / 1e9;   % Tbit
    funs_ant{3} = @(x,y,z) - 0.5 * mymin(r1_3gpp{3}(x) * bw_3gpp, r0_mmw_ant{1}(y,z) * bw_mmw) / 1e9;   % Tbit
...
failIds = ones(Nue, 1);
uavPosArray = zeros(Nue, 3);
Elvs = zeros(1, Nue); Thetas = zeros(1, Nue);
parfor i = 1:Nue
...
%                 finduavpos_mat3(PosUE, PosBS, U, funs, stepSizeMeter, Maps, meterPerPixel, map_x0);
        ...
        if sim_scheme(5)
            ...
            % Record optimal UAV positions
            uavPosArray(i, :) = Xhat3;
            
            % Compute the angle
            uvec = PosUE(1:2) - PosBS(1:2); uvec = uvec / norm(uvec);
            uavpos2d = Xhat3(1:2) - PosBS(1:2);
            Thetas(i) = acos(uavpos2d * uvec' / norm(uavpos2d)) / pi * 180;
            Elvs(i) = atan( max(0, (Xhat3(3) - U.Hbs)) / norm(uavpos2d)) / pi * 180;
            
            % Compute the SNR backoff due to the angles
            Am = 25; phi_3db = 70; theta_etilt = 4; theta_3db = 10; SLAv = 20;

            AH = - min(12 * (Thetas(i) / phi_3db).^2, Am);
            AV = - min(12 * ((Elvs(i) - theta_etilt) / theta_3db).^2, SLAv); % AV = 0;
            AntGain = - min(-(AH + AV), Am);
            
            % Compute the rate
            Xpos2 = PosUE(1:2) + (Xhat3(1:2) - PosUE(1:2)) * U.Hdrone / Xhat3(3);
            los = IsLosK_discrete([PosUE, 0], [Xpos2, U.Hdrone], Maps, meterPerPixel, map_x0);
            Fmin2 = getcostf2d_ant(Xhat3, PosUE, [PosBS, U.Hbs], los, U, funs_ant, AntGain);
...
random_sign = rand(1, size(Rates, 1)); 
random_sign(random_sign > 0.5) = 1;
random_sign(random_sign < 0.5) = -1;
figure(1),
subplot(2,1,1), 
% hist(random_sign .* real(Thetas(validUserId)), 21); 
histogram(random_sign .* real(Thetas(validUserId)), [-31:2:31], 'Normalization', 'pdf'); 
xlabel('Horizontal angle \theta [deg]');  ylabel('Frequency');

subplot(2,1,2), 
histogram(Elvs(validUserId), 30, 'Normalization', 'pdf');
xlabel('Elevation angle \phi [deg]'); ylabel('Frequency');
