% An analysis of the statistical method: Given the BS and user positions,
% the relative optimal UAV position purely depends on the BS-USER distance.
%
% Date: July 20th, 2018.
%
...
if SCENARIO == 3
    ...
    funs{1} = @(x,y) - 0.5 * mymin(r1_mmw{1}(x) * bw_mmw, 0.2 * r0_mmw{1}(y) * bw_mmw) / 1e9;   % Tbit
    funs{2} = @(x,y) - 0.5 * mymin(r1_mmw{2}(x) * bw_mmw, 0.2 * r0_mmw{1}(y) * bw_mmw) / 1e9;   % Tbit
    funs{3} = @(x,y) - 0.5 * mymin(r1_3gpp{3}(x) * bw_3gpp, 0.2 * r0_mmw{1}(y) * bw_mmw) / 1e9;   % Tbit
...
%% show the cost map of the statistical scheme
Npt = 300;
% uid = ceil(rand * 10000),
% PosUE = PosArray(uid, :);
% L = norm(PosUE(1:2) - PosBS(1:2));
% Ls = 20:20:1000;
Ls = 400;
N = length(Ls);
Rho1 = zeros(1, N);
...
for l = 1:N
    L = Ls(l);
    Rhovec = (0:1/Npt:1-1/Npt) * L/2;
    Zvec = (0:1/Npt:1-1/Npt) * L/4;

    [Xmat, Zmat] = meshgrid(Rhovec, Zvec);
    Fmat = zeros(Npt);
    for i = 1:Npt^2
        rho = Xmat(i); z = Zmat(i) + U.Hmin;
    ...
    if l == 1
        % show_map(Rhovec, Zvec, Fmat, [], 1); title('Statistical map'); xlabel('rho'), ylabel('z');colormap(jet(64));
        surf(Xmat, Zmat, Fmat, 'edgecolor', 'none');xlabel('rho'), ylabel('z');colormap(jet(64));view(0,90);
    end
    [fmin, I] = min(Fmat(:));
    rho1 = Xmat(I); z1 = Zmat(I) + U.Hmin;
    Rho1(l) = rho1; Z1(l) = z1; Fmin(l) = fmin;
end
