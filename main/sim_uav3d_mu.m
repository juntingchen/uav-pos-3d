% Modified from sim_uav3d_s3.m: Multiuser case
% Date: July 20th, 2018.
%
...
[MapWidth, MapHeight] = size(Maps{1});
Blds = DATA.BldArea;
urbanMap(urbanMap > 45) = 45;
show_map(Xvec, Yvec, urbanMap, Blds, []);hold on
cmap = jet(64);
bscolor = cmap(60, :);
plot3(PosBS(2), PosBS(1), 50, '^', 'linewidth', 3, 'markersize', 11, 'color', bscolor);
rads = 0:pi/50:2*pi;
plot3(PosUE0(1) + Radius * cos(rads), PosUE0(2) + Radius * sin(rads), 50 * ones(1, length(rads)), ...
