...
%   In this version, we evaluate the performance as a function of the
%   upper bound of the UAV height Hmax. This is a request from the reviewer
%   in the second round review of TWC (first round after the first
%   resubmission.
%   
%   In addition, we also evaluat the performance for several sum power of
%   the UAV.
%   
%   In this script, we foucs on Scenario 3.
%
...
stepSizeMeter = 1;  % UAV search step size
...
uavtxpower_vec = [23, 26, 30];

n_power = length(uavtxpower_vec);
ratePowers = cell(n_power, 1);
tic
for i_power = 1:n_power
    uavtxpower_dBm = uavtxpower_vec(i_power);
    ...
    uavHeights = [45, 60, 80, 100, 120, 150, 200, 300, 400];
    % uavHeights = [45 120 300];
    nHeights = length(uavHeights);
    rateHeights = zeros(nHeights, N_scheme);
    
    for i_case = 1:nHeights

    U.Hmax = uavHeights(i_case);
...
%                   U1 = U;
%                   U1.Hmin = 500; U1.Hmax = 500; U1.Hdrone = 500;
%                   [Fmin2, Xhat2] = ...
%                     finduavpos3d_mat3(PosUE, PosBS, U1, funs, stepSizeMeter, Maps, meterPerPixel, map_x0);
               
%                 % compute the rate
%                 Xhat2 = [Xhat2, U.Hdrone];
%                 los = IsLosK_discrete([PosUE, 0], Xhat2, Maps, meterPerPixel, map_x0);
%                 ks = round((1 - los) * (K - 1) + 1);
% 
%                 rho_opt = norm(Xhat2(1:2) - PosUE(1:2)); z = Xhat2(3);
%                 d1 = sqrt(rho_opt ^ 2 + (z - U.Huser) ^ 2);
%                 d0 = sqrt((norm(PosBS(1:2) - PosUE(1:2), 2) - rho_opt)^2 + (z - U.Hbs)^2);
%                 Fmin2 = funs{ks}(d1, d0);    
...
%             fprintf('error found at user_id = %d\n', i);
    ...
    rateHeights(i_case, :) = mean(Rates, 1);

    end

    ratePowers{i_power} = rateHeights;
end
toc
...
figure(10),
for i = 1:n_power
    p_handle = plot(uavHeights, ratePowers{i}(:, schemes_to_show), 'd-', ...
        'linewidth', 2); hold on
    set(p_handle, {'LineStyle'}, disp_line_styles(i));
end
hold off
gain = rateHeights(end, 5) / rateHeights(1, 5);
title(sprintf('gain = %3.2f', gain));
